...
import { GEMINI_API_KEY, DEBATE_INSTRUCTION } from "../config";
...
function DebateMode({ onBack, user }) {
  const [messages, setMessages] = useState([
    { role: "model", text: "Hello！I am your Tutor。" },
  ]);
  ...
  const handleSend = async () => {
    ...
    const newMessages = [
      ...messages,
      { role: "user", text: displayMessageText },
    ];
    ...
    try {
      ...
      if (!currentChatId) {
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `請幫我為這條訊息生成一句簡短的單一標題使用英文，不超過7個字，不加其他說明：${titlePrompt}`,
                },
              ],
            },
          ],
        ...
        title =
          titleResponse.candidates?.[0]?.content?.parts?.[0]?.text?.slice(
            0,
            50,
          ) || "New Chat";
      ...
      <AppBar position="static" color="warning">
        <Toolbar>
          ...
          <Typography
            variant="h6"
            sx={{ flexGrow: 1 }}
          >{`Debate Mode${chatTitle ? " - " + chatTitle : ""}`}</Typography>
        </Toolbar>
      </AppBar>
          ...
          >
            {messages.map((msg, index) => (
              ...
              >
                ...
                {msg.role === "user" && (
                  <Avatar
                    src={user?.photoURL} // ✨ 使用 Google 帳戶的頭像網址
                    alt={user?.displayName}
                    sx={{ width: 40, height: 40 }} // 你可以調整大小
                  >
                    {/* 如果圖片載入失敗，顯示名字的第一個字作為備案 */}
            ...
            {loading && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography sx={{ color: "#000" }}>Thinking...</Typography>
              </Box>
            )}
